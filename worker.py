...
class WorkerTask:
    """큐 밖에서 예약 상태가 바뀌지 않도록 고정한 작업 정보."""
    ...
    def __post_init__(self) -> None:
        ...
        _validate_number(self.enqueued_at, "enqueued_at")
        ...
        _validate_number(self.accumulated_waiting_time, "accumulated_waiting_time")
    ...
    def for_p2p_transfer(
        ...
        _validate_number(transfer_started_at, "transfer_started_at")
        if transfer_started_at < 0:
            raise ValueError("transfer_started_at은 0 이상이어야 합니다.")
        if transfer_started_at < self.enqueued_at:
            raise ValueError(
                "transfer_started_at은 현재 enqueued_at보다 빠를 수 없습니다."
            )

        _validate_number(new_enqueued_at, "new_enqueued_at")
        if new_enqueued_at < 0:
            raise ValueError("new_enqueued_at은 0 이상이어야 합니다.")
        ...
        try:
            ...
            with self._state_lock:
                ...
                if result["status"] == "UNKNOWN":
                    ...
                    record["status"] = "UNRECOVERABLE_UNKNOWN"
                    response = self.create_message(
                        "P2P_FAILURE", request_id=request_id, transfer_id=transfer_id,
                        source=self.worker_id, target=target_id, reason=reason,
                        communication_ids=communication_ids,
